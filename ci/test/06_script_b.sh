...
if [ "$RUN_FUNCTIONAL_TESTS" = "true" ]; then
  DOCKER_EXEC LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${TEST_RUNNER_ENV}" test/functional/test_runner.py --ci "$MAKEJOBS" --tmpdirprefix "${BASE_SCRATCH_DIR}/test_runner/" --ansi --combinedlogslen=4000 --timeout-factor="${TEST_RUNNER_TIMEOUT_FACTOR}" "${TEST_RUNNER_EXTRA}"
fi
...
if [ "$RUN_FUNCTIONAL_USDT_TESTS" = "true" ]; then
  set -x
  apt update && apt install libcap2-bin -y && capsh --print
  #capsh --supports=CAP_BPF
  #capsh --supports=CAP_PERFMON
  capsh --caps=CAP_SYS_ADMIN+ep -- -c test
  DOCKER_EXEC capsh --caps=CAP_SYS_ADMIN+ep -- -c \'LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${TEST_RUNNER_ENV}" test/functional/test_runner.py --ci "$MAKEJOBS" --tmpdirprefix "${BASE_SCRATCH_DIR}/test_runner/" --ansi --combinedlogslen=4000 --timeout-factor="${TEST_RUNNER_TIMEOUT_FACTOR}" "${TEST_RUNNER_EXTRA}" interface_usdt.py\'
fi
