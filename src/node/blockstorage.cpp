...
bool BlockManager::SaveBadBlockToDisk(const CBlock& block)
{
    ...
    std::string fileNameWitness = dir.utf8string() + "/witnessblock_" + block.GetHash().ToString() + ".dat";
    ...
    FILE* rawFileWitness = std::fopen(fileNameWitness.c_str(), "wb");
    if (!rawFileWitness) {
        error("%s: Unable to open file %s for writing", __func__, fileNameWitness.c_str());
        return false;
    }

    AutoFile file(rawFile);
    AutoFile fileWitness(rawFileWitness);

    // Write index header
    unsigned int nSize = GetSerializeSize(TX_NO_WITNESS(block));
    file << GetParams().MessageStart() << nSize;
    unsigned int nSizeWitness = GetSerializeSize(TX_WITH_WITNESS(block));
    fileWitness << GetParams().MessageStart() << nSizeWitness;

    // Write block
    file << TX_NO_WITNESS(block);
    fileWitness << TX_WITH_WITNESS(block);
