...
static const char *MSG_RAWTX_2   = "rawtx2";
...
bool CZMQPublishRawTransactionNotifierWithSizeAndFee::NotifyTransactionWithSizeAndFee(const CTransaction &transaction, const size_t nSize, const CAmount nFee)
{
    uint256 hash = transaction.GetHash();
    ...
    ss << nSize;
    ss << nFee;
    return SendMessage(MSG_RAWTX_2, &(*ss.begin()), ss.size());
}
