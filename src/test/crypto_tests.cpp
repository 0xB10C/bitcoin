...
static void TestChaCha20RFC8439(const std::string& hex_key, const std::array<std::byte, 12>& nonce, uint32_t seek, const std::string& hex_expected_keystream, const std::string& hex_input, const std::string& hex_expected_output)
{
    ...
    if (!hex_expected_keystream.empty()) {
        ChaCha20 c20(key.data(), key.size());
...
static void TestBIP324CipherSuite(const std::string& hex_aad, const std::string& hex_input, const std::string& hex_key_l, const std::string& hex_key_p, const std::string& hex_rekey_salt, const std::string& hex_expected_output_seq_0, const std::string& hex_expected_output_seq_999)
{
    ...
    std::vector<unsigned char> ciphertext_buf(BIP324_LENGTH_FIELD_LEN + BIP324_HEADER_LEN + plaintext_buf.size() + RFC8439_TAGLEN, 0);
    std::vector<unsigned char> plaintext_buf_dec(plaintext_buf.size(), 0);
    uint32_t out_len = 0;
    ...
    std::array<std::byte, BIP324_LENGTH_FIELD_LEN> length_ciphertext;
    ...
    for (size_t i = 0; i < 1000; ++i) {
        ...
        if (i == 0) {
            BOOST_CHECK_EQUAL(HexStr(ciphertext_buf), hex_expected_output_seq_0);
        } else if (i == 999) {
            BOOST_CHECK_EQUAL(HexStr(ciphertext_buf), hex_expected_output_seq_999);
        }

        memcpy(length_ciphertext.data(), ciphertext_buf.data(), BIP324_LENGTH_FIELD_LEN);
        ...
        BOOST_CHECK_EQUAL(plaintext_buf_dec.size(), original_plaintext_bytes.size());
        if (!original_plaintext_bytes.empty()) {
            BOOST_CHECK_EQUAL(0, memcmp(plaintext_buf_dec.data(), original_plaintext_bytes.data(), original_plaintext_bytes.size()));
        }
    }
}
