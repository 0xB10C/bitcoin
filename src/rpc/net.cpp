...
UniValue AddrmanTableToJSON(const std::vector<std::pair<AddrInfo, AddressPosition>>& tableInfos)
{
    ...
    for (const auto& e : tableInfos) {
        ...
        if (table.exists(std::to_string(location.bucket))) {
            bucket_obj = table[std::to_string(location.bucket)];
        }
        bucket_obj.pushKV(std::to_string(location.position), AddrmanEntryToJSON(info));
        table.pushKV(std::to_string(location.bucket), bucket_obj);
    }
        ...
        RPCResult{
            RPCResult::Type::OBJ_DYN, "", "", {
                {RPCResult::Type::OBJ_DYN, "table", "buckets with addresses in the address manager table ( new, tried )", {
                    {RPCResult::Type::OBJ_DYN, "bucket", "the bucket in the address manager table", {
                        {RPCResult::Type::OBJ, "position", "the position of the address in the bucket", {
                            {RPCResult::Type::STR, "address", "The address of the node"},
                            {RPCResult::Type::NUM, "port", "The port number of the node"},
