...
bool CNetAddr::IsRFC6666() const
{
    return IsIPv6() && HasPrefix(m_addr, std::array<uint8_t, 8>{0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
}

bool CNetAddr::IsRFC5180() const
{
    return IsIPv6() && HasPrefix(m_addr, std::array<uint8_t, 6>{0x20, 0x01, 0x00, 0x02, 0x00, 0x00});
}

bool CNetAddr::IsRFC8215() const
{
    return IsIPv6() && HasPrefix(m_addr, std::array<uint8_t, 6>{0x00, 0x64, 0xFF, 0x9B, 0x00, 0x01});
}

bool CNetAddr::IsRFC9602() const
{
    return IsIPv6() && HasPrefix(m_addr, std::array<uint8_t, 2>{0x5F, 0x00});
}
...
{
    return IsValid() && !(IsRFC1918() || IsRFC2544() || IsRFC3927() || IsRFC4862() || IsRFC6598() || IsRFC5737() || IsRFC1112() || IsRFC4193() || IsRFC4843() || IsRFC7343() || IsRFC6666() || IsRFC5180() || IsRFC8215() || IsRFC9602() || IsLocal() || IsInternal());
}
