...
def main(bitcoind_path):
    ...
    # contain around 2k transactions, requring a buffer size of around 114kB. To cover
...
class Dashboard:
    ...
    def _init_event_rate_win(self):
        ...
        self._event_rate_win = Dashboard.create_win(
            x=3, y=40, height=Dashboard.EVENT_WIN_HEIGHT, width=39, title="Event rate"
        )
        header = " {:<8} {:>8} {:>8} {:>8} "
    ...
    def _update_event_rate(self, rate):
        ...
        row_format = " {:<8} {:>4.1f}tx/s {:>4.1f}tx/s {:>4.1f}tx/s "
        for line, metric in enumerate(["added", "removed", "replaced", "rejected"]):
    ...
    def parse_event(event):
        """Converts events to printable strings"""
        ...
        if type_ == "added":
            return (
                f"{ts} event={type_}, "
                f"txid={bytes(data.hash)[::-1].hex()}, "
                f"vsize={data.vsize}, fee={data.fee}"
            )
        ...
        if type_ == "removed":
            return (
                f"{ts} event={type_}, "
                f"txid={bytes(data.hash)[::-1].hex()}, "
                f'reason={data.reason.decode("UTF-8")}, '
                f"vsize={data.vsize}, fee={data.fee}"
            )
        ...
        if type_ == "rejected":
            return (
                ...
                f'peer_addr={data.peer_addr.decode("UTF-8")}'
            )

        if type_ == "replaced":
            return (
                f"{ts} event={type_}, "
                f"replacement_txid={bytes(data.replacement_hash)[::-1].hex()}, "
                f"replacement_vsize={data.replacement_vsize}, "
                f"replacement_fee={data.replacement_fee}, "
                f"replaced_txid={bytes(data.replaced_hash)[::-1].hex()}, "
                f"replaced_vsize={data.replaced_vsize}, "
                f"replaced_fee={data.replaced_fee}"
            )
