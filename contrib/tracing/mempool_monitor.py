...
int trace_added(struct pt_regs *ctx) {
  ...
  bpf_probe_read_user(&added.hash, sizeof(added.hash), (void *) phash);
...
int trace_removed(struct pt_regs *ctx) {
  ...
  int64_t phash, preason;
  bpf_usdt_readarg(1, ctx, phash);
  bpf_probe_read_user(&removed.hash, sizeof(removed.hash), (void *) phash);
  bpf_usdt_readarg(1, ctx, preason);
  bpf_probe_read_user_str(&removed.reason, sizeof(removed.reason), (void *) preason);
...
int trace_rejected(struct pt_regs *ctx) {
  ...
  int64_t phash, preason;
  bpf_usdt_readarg(1, ctx, phash);
  bpf_probe_read_user(&rejected.hash, sizeof(rejected.hash), (void *) phash);
  bpf_usdt_readarg(1, ctx, preason);
  bpf_probe_read_user_str(&rejected.reason, sizeof(rejected.reason), (void *) preason);
...
int trace_replaced(struct pt_regs *ctx) {
  ...
  int64_t phash, phash_replacement;
  bpf_usdt_readarg(1, ctx, phash);
  bpf_probe_read_user(&replaced.replaced_hash, sizeof(replaced.replaced_hash), (void *) phash);
  ...
  bpf_probe_read_user(&replaced.hash_replacement, sizeof(replaced.hash_replacement), (void *) phash_replacement);
