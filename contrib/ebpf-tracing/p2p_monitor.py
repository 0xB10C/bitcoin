...
""" Interactive bitcoind P2P network traffic monitor based on the
    net:process_message and net:push_message USDTs. """

# This script demonstrates USDT
# tracepoints in Bitcoin Core can enable. This script uses bcc
# (https://github.com/iovisor/bcc) to load a sandboxed program into the linux
# kernel. To load a program into the kernel, root privileges are required.

# Here, this program attaches to two statically defined tracepoints in Bitcoin
# Core. The tracepoint 'net:process_message' is called when a new P2P message
# is received and 'net:push_message' is called on outbound P2P messages.
# The program in the kernel submits the P2P messages to userspace (this script).
...
# BCC: The C program to be compiled to an eBPF programm (by BCC) and loaded into
...
def main(bitcoind_path):
    ...
    # attaching the trace functions defined in bpf_text to the USDTs in bitcoind
    ...
    def handle_inbound(cpu, data, size):
        """ Inboud message handler.
    ...
    def handle_outbound(cpu, data, size):
        """ Inboud message handler.
    ...
    curses.wrapper(loop, bpf, peers)


def loop(screen, bpf, peers):
    screen.nodelay(1)
    selected_list_postion = 0
    win = curses.newwin(30, 70, 2, 7)
    win.erase()
    win.border(ord("|"), ord("|"), ord("-"), ord("-"),
               ord("-"), ord("-"), ord("-"), ord("-"))
    info_panel = curses.panel.new_panel(win)
    info_panel.hide()
    ...
    while True:
        try:
            ...
            ch = screen.getch()
            if (ch == curses.KEY_DOWN or ch == ord("j")) and selected_list_postion < len(
                    peers.keys()) - 1 and info_panel.hidden():
                selected_list_postion += 1
            if (ch == curses.KEY_UP or ch == ord("k")) and selected_list_postion > 0 and info_panel.hidden():
                selected_list_postion -= 1
            if ch == ord('\n') or ch == ord(' '):
                if info_panel.hidden():
                    ...
                    info_panel.hide()
            screen.erase()
            render(screen, peers, selected_list_postion, info_panel)
...
def render(screen, peers, selected_list_postion, info_panel):
    """ renders the list of peers and details panel

    This code is unrelated to USDTs, BCC and BPF.
    """
    header_format = "%5s  %-20s  %-20s  %-22s  %-67s"
    row_format = "%5s  %-5d %9d byte  %-5d %9d byte  %-22s  %-67s"

    screen.addstr(0, 1, (" P2P Message Monitor "), curses.A_REVERSE)
    screen.addstr(
        1, 0, (" navigatate with UP/DOWN or J/K and select a peer with ENTER or SPACE to see individual P2P messages"), curses.A_NORMAL)
    screen.addstr(3, 0,
                  header_format % ("PEER", "OUTBOUND", "INBOUND", "TYPE", "ADDR"), curses.A_BOLD | curses.A_UNDERLINE)
    for i, peer_id in enumerate(sorted(peers.keys())):
        peer = peers[peer_id]
        screen.addstr(i + 4, 0,
                      row_format % (peer_id, peer.total_outbound_msgs, peer.total_outbound_bytes,
                                    peer.total_inbound_msgs, peer.total_inbound_bytes,
                                    peer.connection_type, peer.address),
                      curses.A_NORMAL if i == selected_list_postion else curses.A_DIM)
        if i == selected_list_postion:
            ...
            info_window.addstr(
                1, 1, f"PEER {peer.id} ({peer.address})".center(67), curses.A_REVERSE | curses.A_BOLD)
            info_window.addstr(
                2, 1, f" OUR NODE{peer.connection_type:^48}THEIR NODE ",
                curses.A_BOLD)
