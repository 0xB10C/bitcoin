...
Example scripts for USDT-based tracing
==============================================================

This directory contains scripts showcasing USDT-based tracing on Linux using the
extended Berkeley Packet Filter (eBPF) technology. For more information on USDT
support in Bitcoin Core see the [USDT documentation].

[USDT documentation]: ../docs/usdt.md
...
for complexer tools and daemons and `bpftrace` is prefered for one-liners and
shorter scripts.
...
The bpftrace examples contain a replative path to the `bitcoind` binary. By
...
[Bitcoin Core USDT documentation] on how to list avaliable tracepoints in your
`bitcoind` binary.

[Bitcoin Core USDT documentation]: ../../docs/usdt.md#listing-available-tracepoints
...
privilegdes. Make sure to carefully review any scripts that you run with root
privilegdes first!**
...
By default, `bpftrace` limits strings to 64 byte due the limited stack size in
the kernel VM. For example, Tor v3 addresses exceed the string size limit which
results in the port being cut-off during logging. The string size limit can be
increased wiht the `BPFTRACE_STRLEN` environment variable (`BPFTRACE_STRLEN=70`
works fine).

```
$ bpftrace contrib/ebpf-tracing/log_p2p_traffic.bt
```
...
Per peer inbound and outbound traffic is listed together with connection
information. Peers can be selected individually to view recent P2P messages.

```
$ python3 contrib/ebpf-tracing/p2p_monitor.py ./src/bitcoind
```
...
Showing recent P2P messages between our onde and a their node for a selected peer.
...
third acts as an duration threshold in milliseconds. When the `ConnectBlock()`
...
By default, `bpftrace` limits strings to 64 byte due the limited stack size in
the kernel VM. Block hashes as zero-terminated hex strings are 65 byte which
exceeds the string limit. The string size limit can be set to 65 byte with the
environment variable `BPFTRACE_STRLEN`.

The following command be used to benchmark, for example, `ConnectBlock()`
between height 20000 and 38000 on SigNet while logging all blocks that take
longer than 25ms to connect.

```
$ BPFTRACE_STRLEN=65 bpftrace contrib/ebpf-tracing/connectblock_benchmark.bt 20000 38000 25
```
