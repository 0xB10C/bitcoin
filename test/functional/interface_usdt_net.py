...
int trace_outbound_connection(struct pt_regs *ctx) {
    ...
    bpf_usdt_readarg(6, ctx, &outbound.existing);
    outbound_connections.perf_submit(ctx, &outbound, sizeof(outbound));
    return 0;
};

BPF_PERF_OUTPUT(evicted_connections);
int trace_evicted_connection(struct pt_regs *ctx) {
    ...
    evicted_connections.perf_submit(ctx, &evicted, sizeof(evicted));
    return 0;
};
...
class NetTracepointTest(BitcoinTestFramework):
    ...
    def outbound_conn_tracepoint_test(self):
        ...
        checked_outbound_connections = 0

        def handle_outbound_connection(_, data, __):
            nonlocal checked_outbound_connections
            event = ctypes.cast(data, ctypes.POINTER(NewConnection)).contents
            self.log.info(f"handle_outbound_connection(): {event}")
            assert(event.conn.id > 0)
            assert(event.conn.net_group > 0)
            assert_equal(EXPECTED_CONNECTION_TYPE,
                         event.conn.conn_type.decode('utf-8'))
            assert_equal(0, event.conn.network)
            checked_outbound_connections += 1
        ...
        assert_equal(EXPECTED_OUTBOUND_CONNECTIONS,
                     checked_outbound_connections)
        for node in testnodes:
            node.peer_disconnect()

    def evicted_conn_tracepoint_test(self):
        self.log.info("hook into the net:evicted_connection tracepoint")
        ctx = USDT(pid=self.nodes[0].process.pid)
        ctx.enable_probe(probe="net:evicted_connection",
                         fn_name="trace_evicted_connection")
        bpf = BPF(text=net_tracepoints_program, usdt_contexts=[ctx], debug=0)
        ...
        checked_evicted_connections = 0

        def handle_evicted_connection(_, data, __):
            nonlocal checked_evicted_connections
            event = ctypes.cast(data, ctypes.POINTER(
                ClosedConnection)).contents
            self.log.info(f"handle_evicted_connection(): {event}")
            assert(event.conn.id > 0)
            assert(event.conn.net_group > 0)
            assert_equal("inbound", event.conn.conn_type.decode('utf-8'))
            assert_equal(0, event.conn.network)
            assert(event.time_established > 0)
            checked_evicted_connections += 1

        bpf["evicted_connections"].open_perf_buffer(handle_evicted_connection)
        ...
        assert_equal(EXPECTED_EVICTED_CONNECTIONS, checked_evicted_connections)
        for node in testnodes:
            node.peer_disconnect()
    ...
    def misbehaving_conn_tracepoint_test(self):
        ...
        checked_misbehaving_connections = 0
        misbehaving_score = 0

        def handle_misbehaving_connection(_, data, __):
            nonlocal checked_misbehaving_connections, misbehaving_score
            event = ctypes.cast(data, ctypes.POINTER(
                MisbehavingConnection)).contents
            self.log.info(f"handle_misbehaving_connection(): {event}")
            assert(event.id > 0)
            assert(event.howmuch > 0)
            assert(len(event.message) > 0)
            assert_equal(misbehaving_score, event.score_before)
            misbehaving_score += event.howmuch
            assert_equal(
                misbehaving_score >= MISBEHAVING_DISCOURAGEMENT_THRESHOLD, event.threshold_exceeded)

            checked_misbehaving_connections += 1

        bpf["misbehaving_connections"].open_perf_buffer(
            handle_misbehaving_connection)
