...
class NetTest(BitcoinTestFramework):
    ...
    def test_getaddrmaninfo(self):
        ...
        self.log.debug("Test that the newly added addresses appear in the verbose output")
        res = node.getaddrmaninfo(verbose=True)
        ...
        assert_equal(len(res["new_table"]), res["all_networks"]["new"])
        added_entry = list(
            filter(lambda e: "2.4.8.16" in e["address"], res["new_table"]))[0]
        ...
        assert_equal(len(res["tried_table"]), res["all_networks"]["tried"])
        added_entry = list(
            filter(lambda e: "2.4.8.16" in e["address"], res["tried_table"]))[0]
