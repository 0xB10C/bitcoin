...
int trace_utxocache_add(struct pt_regs *ctx) {
    ...
    bpf_probe_read_user(&add.txid, sizeof(add.txid), (void *) ptxid);
...
int trace_utxocache_spent(struct pt_regs *ctx) {
    ...
    uint64_t ptxid;
    bpf_usdt_readarg(1, ctx, &ptxid);
    bpf_probe_read_user(&spent.txid, sizeof(spent.txid), (void *) ptxid);
...
int trace_utxocache_uncache(struct pt_regs *ctx) {
    ...
    uint64_t ptxid;
    bpf_usdt_readarg(1, ctx, &ptxid);
    bpf_probe_read_user(&uncache.txid, sizeof(uncache.txid), (void *) ptxid);
