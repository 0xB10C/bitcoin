...
class ZMQTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        try:
            self.test_basic()
            self.test_reorg()
            self.test_rawblockconnected()
            self.test_rawblockconnected_reorg()
        finally:
    ...
    def test_rawblockconnected(self):
        import zmq
        address = 'tcp://127.0.0.1:28333'
        socket = self.ctx.socket(zmq.SUB)
        socket.set(zmq.RCVTIMEO, 60000)
        rawblockconnected = ZMQSubscriber(socket, b'rawblockconnected')

        self.log.info("Test patched rawblockconnected topic")

        # rawblockconnected should notify for every block connected
        self.restart_node(0, ['-zmqpub%s=%s' % (rawblockconnected.topic.decode(), address)])
        socket.connect(address)
        # Relax so that the subscriber is ready before publishing zmq messages
        sleep(0.2)
        ...
        for block_hash in genhashes_node0:
            block = rawblockconnected.receive()
    ...
    def test_rawblockconnected_reorg(self):
        import zmq
        address = 'tcp://127.0.0.1:28333'
        socket = self.ctx.socket(zmq.SUB)
        socket.set(zmq.RCVTIMEO, 60000)
        ...
        self.log.info("Test patched rawblockconnected topic with reorg")
