...
int trace_replaced(struct pt_regs *ctx) {
  ...
  uint64_t phash, preplacement_hash;
  bpf_usdt_readarg(1, ctx, &phash);
  bpf_probe_read_user(&rejected.hash, sizeof(rejected.hash), (void *) phash);
  ...
  bpf_probe_read_user(&replaced.replacement_hash, sizeof(replaced.replacement_hash), (void *) preplacement_hash);
